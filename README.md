...
## Чтение каналов: два режима

### Режим 1: Public web fallback (работает без регистрации API)

Бот умеет читать **публичные** Telegram-каналы через `https://t.me/s/<channel>` без Telethon и без API-ключей.

- **Требования:** никаких — работает сразу.
- **Ограничения:** только публичные каналы; Telegram может ограничить доступ к странице.
- **Для приватных каналов** нужен Telethon (см. ниже).

Этот режим используется автоматически, когда `TELETHON_ENABLED=false`.

---

### Режим 2: Telethon (лучший способ, поддерживает приватные каналы)
...
> **Примечание:** если my.telegram.org не даёт создать приложение — используйте режим Public web fallback для открытых каналов.
